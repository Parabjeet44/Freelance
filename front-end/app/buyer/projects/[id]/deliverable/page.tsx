...
export default function ProjectDetailsPage() {
  ...
  useEffect(() => {
    const fetchProjectAndDeliverable = async () => {
      try {
        // Fetch project
        const projectRes = await axios.get(`http://localhost:5000/api/project/projects/${id}`, {
        ...
        try {
          const deliverableRes = await axios.get(
            `http://localhost:5000/api/deliverable/projects/${id}/deliverables`,
            { withCredentials: true }
          )
          const deliverable = deliverableRes.data.deliverable
          deliverableUrl = deliverable?.fileUrl || deliverable?.link || null
        } catch (deliverableErr: any) {
          console.warn('No deliverable found:', deliverableErr.response?.data?.error || deliverableErr.message)
        }
